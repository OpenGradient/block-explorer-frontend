...
const WorkflowsTable = ({ items, page, isLoading, sorting, setSorting, top }: Props) => {
    ...
    <Table>
      ...
      <Tbody>
        { items.map((item, index) => (
          <WorkflowsTableItem key={ item.address + (isLoading ? index : '') } token={ item } index={ index } page={ page } isLoading={ isLoading }/>
        )) }
      </Tbody>
    </Table>
  );
